...
class Application(tk.Frame):
    ...
    def widget(self):
        self.intro = tk.Label(self.master, text = ' Guess my number; 4-digit, non-duplicates ', font = ('Cascadia Code', 12))
        self.intro.grid(row = 1, column = 1)
        self.box = tk.Entry(self.master, width = 25)
        self.box.grid(row = 1, column = 2)
        self.box.focus_set()
        self.note = tk.Label(self.master, text = ' Enter to guess, Esc to quit', font = ('Cascadia Code', 10))
    ...
    def mastermind(self, event):
        ...
        while True:
            ...
            if self.guess.isnumeric() == False:
                ...
                self.info = tk.messagebox.showinfo('', 'Not Enough!')
                self.box.delete(0, tk.END)
                break

            elif len(self.guess) > 4:
                self.info = tk.messagebox.showinfo('', 'Too many!')
                self.box.delete(0, tk.END)
                break
