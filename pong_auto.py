...
def main():
    ...
    while True:
        ...
        # プレイヤーのスコア表示を設定
        score1_show = score_font.render('COM 1: ' + str(score1), True, (255, 255, 255))
        score1_pos = score1_show.get_rect()
        score1_pos.centerx = int(WIDTH / 4)
        score1_pos.centery = 40

        # COM のスコア表示を設定
        score2_show = score_font.render('COM 2: ' + str(score2), True, (255, 255, 255))
        ...
        # COM の挙動を設定
        # COM ラケットがボールより上にあったら下に移動
        # COM ラケットがボールより下にあったら上に移動
        # ボールが遠くにあったら早く移動
        if racket1.centery < ball.centery and racket1.bottom < HEIGHT - 5 and ball.centerx < int(WIDTH / 2):
            racket1.centery += CPU_SWING
            if racket1.centery < ball.centery - int(HEIGHT / 8) and ball.centerx < int(WIDTH / 2):
                racket1.centery += CPU_SWING + randint(-3, 3)
        elif racket1.centery > ball.centery and racket1.top > 5 and ball.centerx < int(WIDTH / 2):
            racket1.centery -= CPU_SWING
            if racket1.centery > ball.centery + int(HEIGHT / 8) and ball.centerx < int(WIDTH / 2):
                racket1.centery -= CPU_SWING + randint(-3, 3)

        if racket2.centery < ball.centery and racket2.bottom < HEIGHT - 5 and ball.centerx > int(WIDTH / 2):
            racket2.centery += CPU_SWING
            if racket2.centery < ball.centery - int(HEIGHT / 8) and ball.centerx > int(WIDTH / 2):
                racket2.centery += CPU_SWING + randint(-3, 3)
        elif racket2.centery > ball.centery and racket2.top > 5 and ball.centerx > int(WIDTH / 2):
            racket2.centery -= CPU_SWING
            if racket2.centery > ball.centery + int(HEIGHT / 8) and ball.centerx > int(WIDTH / 2):
                racket2.centery -= CPU_SWING + randint(-3, 3)
        ...
        # プレイヤーか COM が勝利条件を満たしたとき、メッセージを画面に描画
        if victory_1:
